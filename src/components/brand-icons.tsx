...
/**
 * SNSのブランドアイコン。lucide-react v1 がブランドアイコンを廃止したため、
 * `public/images/brand/` に置いた公式SVGをそのままコンポーネント化している。
 *
 * 原典から変えたのは2点だけ。
 * - `fill` を `currentColor` に置換（親の文字色 = トークン経由で色が決まる）
 * - `width` / `height` を外して、サイズはクラス側（`size-*`）に委ねる
 * パスと viewBox は一切触らない。
 */

export type BrandIcon = (props: SVGProps<SVGSVGElement>) => React.JSX.Element;
